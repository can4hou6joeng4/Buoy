...
相关项目：[Tide](https://github.com/can4hou6joeng4/Tide) —— 掘金每日签到与抽奖播报。
    ...
    steps:
      - name: 执行签到
        ...
        with:
          ...
          # 可选：通知触发条件，默认在失败、任意账号余额变化、新增账号首次建立基线时通知
          notify-triggers: failed,balance_changed,first_seen
          # ... 其他通知配置
```
...
- `balance_changed`：任意账号检测到实际余额变化
- `first_seen`：任意新增账号首次成功获取余额并建立基线
- `failed`：任意账号签到失败
- `success`：任意账号签到成功
- `always`：总是发送
- `never`：从不发送

[默认配置](src/notif/trigger_manager.py#L11)为 `NOTIFY_TRIGGERS='failed,balance_changed,first_seen'`，即账号失败、任意账号余额变化、或新增账号首次建立余额基线时发送提醒；日常全部成功且余额无变化时不发送提醒。

Fork 本仓库内置的 workflow 默认读取仓库变量 `NOTIFY_TRIGGERS`，若未配置则回退到 `failed,balance_changed,first_seen`。您也可以在 Composite Action 中通过 `notify-triggers` 输入覆盖默认值。

您可通过设置该环境变量达到自定义通知时机的目的，比如将 `NOTIFY_TRIGGERS` 设置为 `failed,balance_changed,first_seen`，则会在账号失败时立即提醒，在任意账号余额变化时发送成功汇总，并在新增账号首次验证成功时发送提醒。
...
- `first_seen_accounts`: 首次成功获取余额并建立基线的新增账号列表
...
Telegram 默认模板采用账号明细优先的紧凑排版和语义化 Emoji：保留执行时间，每个成功账号用一行展示账号名、当前额度和已用额度，不再在字段之间插入空白行。变化模式不一致时，变化值直接追加在对应账号行；多个账号具有完全相同的变化模式时，只追加一行共同变化摘要。底部使用单行签到统计，并在全部成功时保留庆祝提示。
